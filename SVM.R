...
a <- confusionMatrix(factor(ifelse(scaled.svm.pred > 0.5, 1, 0)), factor(scaled.test.df$Y))
print(a$overall[1])
#accuarcy 0.7113
...
ab <- confusionMatrix(factor(ifelse(scaled.svm2.pred > 0.5, 1, 0)), factor(scaled.test.df$Y))
ab
#accuracy 0.7126
...
abc <- confusionMatrix(factor(ifelse(scaled.svm.pred > 0.5, 1, 0)), factor(scaled.test.df$Y))
abc
